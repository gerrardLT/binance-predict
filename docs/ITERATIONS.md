...
## 2026-09-15 实盘表现与均值回归诊断 Phase 0 `pending commit`

**问题** 原盈利趋势只有累计胜率并统一对照 50%，无法识别 rolling 漂移、低胜率高赔率通道的真实保本线，也把 G7 等同窗嵌套订单当成独立样本。
**改动** 新增只读 performance summary/channel diagnostics API、冻结通道基准注册表与三层前端工作区；按通道展示 rolling 20/50、EWMA、Wilson/Jeffreys 区间、实现 EV、动态保本率和单维风险归因。
**口径** 当前注册通道内真实投入/PnL 按所有订单全计；组合净盈利率按 `market_id`、缺失时按 `market_period+window_start` 聚合，同窗净 PnL 判赢，结算冲突只排除窗口胜率、不吞真实资金。
**安全** 功能只读，不调用通道 toggle、不改变金额或门禁；实际胜负减保本概率只称样本报价边际，不冒充预测校准，也不因连败提供加仓或放宽门禁建议。
**验证** 新增统计/API 测试后相关测试 `222 passed`；全量后端测试退出码 0；前端 lint 无新增错误、`npm run build` 成功（Node 22.11 低于 Vite 建议 22.12，但未阻断）。
**遗留** Phase 0 的趋势、波动率和部署版本没有订单时点事实时统一为 `LEGACY_UNKNOWN`；需 Phase 1 捕获严格 ex-ante 特征、不可变部署标识与逐笔 forecast probability 后，才上线 Brier/ECE 真校准。

---

## 2026-09-14 统一影子执行评估与实盘对比漏斗 `pending commit`

**问题** 旧影子面板只统计理论信号按影子报价成交的固定 1U 已实现收益，未包含实盘策略门、通道配置、日限/防重/互斥、市场、余额、真实报价、护栏、成交和结算，理论结果不能代表实盘 EV。
**改动** 新增覆盖 44 个登记版本的统一 assessment 账本、五类 source projector、订单/结算关联、比较 API 与前端三层面板；projector 使用 DB anti-join，旧订单对账在批次限制前按 registry 过滤，重启不依赖内存水位。
**安全** 现有五张影子表仍是理论事实源；执行观测复用真实交易链已有数据，不增加 Binance 请求，不修改通道开关、资金、护栏或 LIMIT 行为；拒绝事实单事务落账，失败不阻断下单。
**口径** 理论与可执行层使用固定 1U 已实现收益，实际层仅统计带非空 exchange `order_id` 且按该 ID 全局去重的订单，使用 USDT PnL 和 `sum(pnl)/sum(stake)` 本金加权 ROI；筛选后的漏斗、收益、原因和累计曲线统一重算。
**验证** 全量后端 `1194 passed, 14 skipped`、前端构建与 375px/1366px 真 Chrome 响应式断言通过；专用本地 PostgreSQL 空库完成全链 upgrade → downgrade → upgrade，新表/列/FK 往返及 44 版本/5 source 覆盖审计均通过。
**遗留** 旧历史的瞬时配置、盘口、余额和互斥事实不可重建，保持 `LEGACY_UNKNOWN`；默认本地开发库存在既有 schema 超前于 Alembic revision 的漂移，本次未用 stamp 或手工 DDL 掩盖，生产迁移仍只随 CI 部署执行。
...
**改动** 根据 40d 全样本严格回测结果，在 `src/binance_predict/main.py` 的 `SHADOW_BENCH` 中填充 G0~G4 及 G7 全系变体的基准胜率与平均 EV 点估计。
**验证** 运行 `tests/test_signals_analytics_api.py` 20 项测试全过，前端 `npm run build` 打包通过无报错。
**遗留** 基准基于 40d 历史全样本离线回测，前向运行后实际表现以影子信号及实盘结算累计值为准。

---

## 2026-09-13 解除 firsthit 全族同窗互斥支持 G7 各变体独立实盘测试 `a77e314`
...
**改动** 新增 `notification_channel_overrides` + 运行时闸：全局微信/邮件、逐通道总开关、雷达/成交/弃单/结算、字段显隐；默认全开。场景邮件旁路一并接入。修补 commit 后显式 `session.refresh(row, ["updated_at"])`。
**改动** 前端「通知配置」页：物理通道状态（不回凭据）、测试微信/邮件、逐信号矩阵与批量全开/全关/恢复默认。
**验证** 本地 15 个专项单元测试全过；生产环境 HTTP API 验证全量 27 通道 PUT 保存、单通道覆盖、单通道 reset 与全局 reset 均返回 200。
**遗留** 邮件仍只覆盖既有结算复盘，不新造雷达/成交邮件；多 worker 靠 60s 刷新收敛。
