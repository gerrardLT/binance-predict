...
## 2026-09-13 逐信号通知配置（微信/邮件事件与字段） `pending commit`

**问题** 通知只有 .env 总开关，无法按信号、事件、字段配置 WxPusher 与结算邮件。
**改动** 新增 `notification_channel_overrides` + 运行时闸：全局微信/邮件、逐通道总开关、雷达/成交/弃单/结算、字段显隐；默认全开。场景邮件旁路一并接入。
**改动** 前端「通知配置」页：物理通道状态（不回凭据）、测试微信/邮件、逐信号矩阵与批量全开/全关/恢复默认。
**验证** 专项测试覆盖默认全开、通道隔离、字段显隐、未知通道放行、场景邮件闸、API 鉴权与 422。
