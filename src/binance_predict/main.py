...
rev2_inside_5m_shadow_detector: Rev2InsideShadowDetector | None = None
...
async def lifespan(app: FastAPI):
    ...
    if settings.rev2_inside_shadow_enabled:
        rev2_inside_shadow_detector = Rev2InsideShadowDetector(
            collector=collector,
            pm_latest=_pm_15m_latest,
            timeframe="15m",
        )
        rev2_inside_5m_shadow_detector = Rev2InsideShadowDetector(
            collector=collector,
            pm_latest=_pm_market_info,
            timeframe="5m",
        )
        await rev2_inside_shadow_detector.start()
        await rev2_inside_5m_shadow_detector.start()
        logger.info(
            "Rev2Inside 双周期检测器已启动（15m HM/IH 短影≤10%；"
            "5m HM 短上影≤10%且主下影[75%,90%)；影子落表 + 实盘钩子挂载中）"
        )
    ...
    if multi_live_trader is not None:
        ...
        if rev2_inside_5m_shadow_detector is not None:
            rev2_inside_5m_shadow_detector._on_live_fire = multi_live_trader.on_nextbar_signal
    ...
    # 停止 Rev2 5m / 15m 孕线反转影子检测器
    if rev2_inside_shadow_detector is not None:
        await rev2_inside_shadow_detector.stop()
    if rev2_inside_5m_shadow_detector is not None:
        await rev2_inside_5m_shadow_detector.stop()
    # 停止反转形态影子检测器
...
async def live_pnl_curve(
    ...
    口径（2026-09-08 审计固化，响应 scope 字段同步下发）：
    trade_orders 中 signal_version 属于通道注册表（LIVE_CHANNELS）、
    FILLED 且 win/pnl 非空的行，按 window_start 时序逐单累计本地估算
    pnl（USDT）。**不含**：manual_test、已退役通道的历史订单、
    NOISE/EXPIRED（win=NULL）、FAILED/未成交/未结算——因此本端点
    total 与「资金变化」（/api/trades/fund-flow 全量订单派生）口径不同：
    前者是「当前注册通道的方向性已结算收益」，后者含历史所有已落库
    订单（含退役通道与 NOISE/EXPIRED）。两数字并存是设计而非 bug，
    前端已标注各自口径，禁止混用对比。
    ...
    return {
        ...
        "scope": {
            "filter": "LIVE_CHANNELS ∩ FILLED ∩ win≠NULL ∩ pnl≠NULL",
            "excludes": ["manual_test", "已退役通道历史订单", "NOISE/EXPIRED",
                         "FAILED/未成交/未结算"],
            "pnl_basis": "本地估算（赢=shares−成本优先，回退 amount/均价−amount；输=−投入）",
            "note": "与 /api/trades/fund-flow 的全量口径并存，禁止混用对比",
        },
    }
...
SHADOW_BENCH: dict[str, tuple[float | None, float | None, str]] = {
    ...
    "firsthit_down_g4_v1": (0.157, 1.265, "首触交互G4: G0+body_r≤0.35∧chg≤+2.82bp（即G1∩G3；全样本300次胜率15.7%/EV+1.265，BH-FDR q=0.063 STRICT_PASS；⚠G4⊂G1且⊂G3非独立暴露，同窗三门全中=同一注重复下注；⚠前向功效偏紧：calib EV与前向CI半宽接近，通过概率约50%，FAIL应延长观察期而非直接否决；前向现算裁决）"),
    # 首触反转 G7 系列（2026-09-08 优化衍生）：
    "firsthit_down_g7_v1": (0.192, 2.213, "首触G7基底: G0+body_r≤0.35∧wick=1（全样本260次胜率19.2%/EV+2.213，FDR q=0.0002；前向现算裁决）"),
    "g7_streak_v1": (0.193, 2.503, "首触G7非强连阳: G7+前驱5m连阳≤1（全样本202次胜率19.3%/EV+2.503，FDR q=0.0004；前向现算裁决）"),
    "g7_wick20_v1": (0.205, 2.358, "首触G7长上影: G7+upper_wick≥2bp（全样本127次胜率20.5%/EV+2.358；前向现算裁决）"),
    "g7_strict_v1": (0.219, 2.998, "首触G7严格版: G7+streak≤1∧wick≥1.5bp（全样本114次胜率21.9%/EV+2.998；前向现算裁决）"),
    "g7_q05_v1": (0.137, 5.245, "首触G7深折价: G7+q≤0.05（极端赔率凸性档，全样本51次胜率13.7%/EV+5.245；前向现算裁决）"),
    "g7_t270_v1": (0.286, 4.047, "首触G7非极晚: G7+t≤270s（全样本84次胜率28.6%/EV+4.047；前向现算裁决）"),
    ...
    # 逆向选择 + 热调护栏 0.20 后 8 天零成交）；现 MARKET 下单，护栏 0.30 = 弃单线（含贴线）。
    # bench 胜率=720d 纯 K 线次根反转结算基准点估计；EV 由真实报价前向现算
