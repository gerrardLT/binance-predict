...
const SIGNAL_INFO: Record<string, { name: string; kind: '实盘' | '影子' | '场景'; desc: string; liveOk?: boolean; retired?: boolean }> = {
  ...
  hm_inside_15m_v2: {
    ...
    desc: '15m Ver2 经典反转：前置 4 根高点最长实体阳线 + 信号柱完全被包裹（Inside Bar）+ 顶部长下影上吊线 → 押次根 15m 收阴 DOWN（720d 胜率 56.6%，30d 胜率 65.4%）。最佳挂单护栏 0.30（单笔期望 EV +18.3%，成交率 65.7%）。',
  },
  ih_inside_15m_v2: {
    name: '15m孕线倒垂线反转（押UP）', kind: '影子', liveOk: true,
    desc: '15m Ver2 经典反转：前置 4 根低点最长实体阴线 + 信号柱完全被包裹（Inside Bar）+ 底部长上影倒垂线 → 押次根 15m 收阳 UP（720d 胜率 52.5%，30d 胜率 62.5%）。最佳挂单护栏 0.30（单笔期望 EV +18.3%，成交率 65.7%）。',
  },
}
